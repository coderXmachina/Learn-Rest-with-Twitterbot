...
var namingConvInc = 13; //for downloading
...
var getMyAPOD = function(){
    ...
    var g = new Date(2016, 8, testDate); //this is the date
    var APODTextDescitp = '';
    ...
    apod(g , function(err, mydata) { //year, month, day
        if(err){
            ...
            if(mydata.media_type == 'image'){//proceed with archiving
                ...
                fs.appendFile('APOD Archive.json', APODTextDescitp, function(err){
                    if(err) throw err;
                    console.log('APOD Archived')
                });

                download(mydata.url, namingConvention, function() {
                    console.log("\n" + namingConvention + ' APOD Downloaded.\n\n\n')
                });
            }
...
getMyAPOD();
setInterval(getMyAPOD, 240000); //every two minutes give me an apod
